...
class GraphEncoder(Module):
    def __init__(self,graph,device, entity, emb_size, kg, embeddings=None, fix_emb=True, seq='rnn', gcn=True, hidden_size=100, layers=1, rnn_layer=1,u=None,v=None,f=None):
        ...
        self.kg = kg
        self.user_length = len(self.kg.G['user'])
        self.item_length = len(self.kg.G['item'])
        ...
        self.conv1 = dglnn.HeteroGraphConv({rel: dglnn.GATv2Conv(emb_size, hidden_size//4, 4) for rel in rel_names},
                                           aggregate='mean')
    ...
    def forward(self, b_state):
        ...
        h1 = {k: self.relu(v.view(v.shape[0],-1)) for k, v in h1.items()}
        ...
        for s, o in zip(b_state, batch_output):
            eps = self.sigmoid(self.gate1(o[:len(s['cur_node']), :][None, :])+self.gate2(gnn_embedding[s['cur_node']]))
            seq_embeddings.append((1 - eps) * o[:len(s['cur_node']), :][None, :] + eps * gnn_embedding[s['cur_node']])
