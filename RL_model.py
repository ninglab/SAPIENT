...
def train(args, kg, dataset, filename):
    ...
    buffer = {x:[[],15] for x in range(args.num_rollouts)}
    for train_step in range(args.load_rl_epoch+1, args.max_steps+1):
        ...
        for i_episode in tqdm(range(args.sample_times),desc='sampling'):
            ...
            while not done:
                ...
                if done:
                    next_state = None
                temp = (state, action, next_state, reward, next_cand)
                buffer[i_episode%args.num_rollouts][0].append(copy.deepcopy(temp))
                state = next_state
                cand = next_cand
                t = count
                count+=1
            ...
            if (i_episode+1)%args.num_rollouts==0:
                ...
                for i in range(args.inner_epoch):
                    newloss = agent.optimize_model(args.batch_size, args.gamma, env)
...
def main():
    ...
    parser.add_argument('--inner_epoch', type=int, default=1, help='inner_epoch for optimizing the q network.')
    ...
    parser.add_argument('--sample_times', type=int, default=1000, help='the epoch of sampling')
