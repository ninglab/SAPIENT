...
def dqn_evaluate(args, kg, dataset, agent, filename, i_episode):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn,
                                       cand_num=args.cand_num, cand_item_num=args.cand_item_num, attr_num=args.attr_num, mode='test', entropy_way=args.entropy_method,
    ...
    for user_num in tqdm(range(user_size)):
        ...
        for t in count():
            if t == 14:
                is_last_turn = True
            action, sorted_actions,_ = agent.select_action(state, cand, action_space, test_env, is_test=True, is_last_turn=is_last_turn)
        ...
        if (user_num+1) % args.observe_num == 0 and user_num > 0:
            ...
            print('SR5:{}, SR10:{}, SR15:{}, AvgT:{}, Rank:{}, reward:{} '
                  'Total epoch_user:{}'.format(SR5 / args.observe_num, SR10 / args.observe_num, SR15 / args.observe_num,
            ...
            if args.eval_num != 1:
                ...
                print('###mean### SR5:{}, SR10:{}, SR15:{}, AvgT:{}, Rank:{}, reward:{}'.format(SR5_mean, SR10_mean, SR15_mean, AvgT_mean, Rank_mean, reward_mean))
            SR5, SR10, SR15, AvgT, Rank, total_reward = 0, 0, 0, 0, 0, 0
            SR_turn_15 = [0] * args.max_turn
            tt = time.time()
        enablePrint()
